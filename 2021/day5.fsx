...
let linePoints {From = (x0, y0); To = (x1, y1)} =
  let xs = (if x0 < x1 then seq {for x in x0..x1 -> x}
    elif x0 = x1 then Seq.initInfinite (fun _ -> x0) 
    else seq {for x in x0.. -1 ..x1 -> x})
  let ys = (if y0 < y1 then seq {for y in y0..y1 -> y}
    elif y0 = y1 then Seq.initInfinite (fun _ -> y0)
    else seq {for y in y0.. -1 ..y1 -> y})
  Seq.zip xs ys |> List.ofSeq

test |> removeDiagonals |> List.collect linePoints |> countPoints

let countPoints points =
  (Map.empty, points)
  ||> List.fold (fun counter pt ->
                 counter
                 |> Map.change pt (fun c ->
                   match c with
                   | Some c -> Some (c + 1)
