...
(define (add-sleep! minutes start stop)
  (for ([min (in-range start stop)])
    (vector-set! minutes min (add1 (vector-ref minutes min)))))

(define (parse-log stamps)
  (define start-regex (pregexp "\\[(.*?)\\] Guard #(\\d+)"))
  (define sleep-regex (pregexp "\\[.*?:(\\d+)\\] falls asleep"))
  (define wake-regex (pregexp  "\\[.*?:(.\\d+)\\] wakes up"))
  (define log (make-hash))
  (for/fold ([guard #f]
             [sleep-start #f])
            ([stamp stamps])
    (define-values (start sleep wake)
      (values (regexp-match start-regex stamp)
              (regexp-match sleep-regex stamp)
              (regexp-match wake-regex stamp)))
    (match (list start sleep wake)
      ;; At shift start update the log and reset intervals
      [(list (list _ t n) #f #f) (let ([new-guard (string->number n)])
                                   (hash-update! log new-guard values (make-vector 60 0))
                                   (values new-guard #f))]
      ;; When guard sleeps, set the sleep time
      [(list #f (list _ t) #f) (values guard (string->number t))]
      ;; When guard wakes, add to the interval
      [(list #f #f (list _ t)) (begin
                                 (add-sleep! (hash-ref log guard) sleep-start
                                             (string->number t))
                                 (values guard #f))]))
  log)

(define (sleep-totals log)
  (for/list ([(guard minutes) (in-hash log)])
    (cons guard (apply + (vector->list minutes)))))

(define (max-sleeper log)
  (car (sort (sleep-totals log) > #:key cdr)))

(define (sleepiest-minute minutes)
  (for/fold ([max 0]
             [max-i 0])
            ([val (in-vector minutes)]
             [i (in-range (vector-length minutes))])
    (if (> val max)
        (values val i)
        (values max max-i))))

(define (get-answer1 timestamps)
  (define log (parse-log timestamps))
  (define guard (car (max-sleeper log)))
  (define-values (_ min) (sleepiest-minute (hash-ref log guard)))
  (* guard min))

(define (get-answer2 timestamps)
  (define log (parse-log timestamps))
  (define-values (max guard minute)
    (for/fold ([max 0]
               [winner #f]
               [minute #f])
              ([(guard minutes) (in-hash log)])
      (define-values (tot min) (sleepiest-minute minutes))
      (if (> tot max)
          (values tot guard min)
          (values max winner minute))))
  (* guard minute))
...
(module+ test
  ...
  (define log (parse-log timestamps))
  (check-equal? (max-sleeper log) (cons 10 50))
  (define-values (total min) (sleepiest-minute (hash-ref log 10)))
  (check-eq? total 2)
  (check-eq? min 24)
  (check-eq? (get-answer1 timestamps) 240)
  (check-eq? (get-answer2 timestamps) 4455))


(define timestamps (sort (list
